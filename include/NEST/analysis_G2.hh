...
bool PrintSubThr = false; //BEWARE that false makes #evts < your req
...
double minS1 = 2.5;  // units are controlled by the usePD flag
// this is separate from S1 thresholds controlled by detector
double maxS1 = 120.5;
int numBins = 118;  // for LUXRun03 DD, change these to 1.7,110.6,99
...
// leakage frac 1 outputs GoF for sim band 1st cf. data band 2nd (Gauss
// centroids of hist in S1 slices) 2 outputs wimp masses and cross-sections for
// given efficiency. 3 outputs SI cross-section using adaptive cut and count. 
// args are data file, wimp spectrum file (e.g. from execNEST), and reference 
// cross section
int NRbandCenter =
